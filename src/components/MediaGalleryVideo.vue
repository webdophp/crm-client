<template>
    <v-container fluid class="px-0 py-0">
        ...
        <v-dialog v-model="dialogVideo" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark color="primary">
                    <v-btn icon dark @click="cancelVideo(selectId)">
                        <v-icon>mdi-close</v-icon>
                    </v-btn>
                    <v-toolbar-title>{{ $t('select_videos') }}</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="saveVideo(selectId)">{{ $t('save') }}</v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <v-row v-resize="onResize" align="center" justify="center">
                        <v-col cols="12">
                            <v-text-field ref="sourceVideo" v-model="videoUrl" type="text"
                                          @change="setupVideoPreview" prepend-icon="mdi-image-edit"
                                          :label="$t('url_video_from_youtube')"
                                          color="primary" required></v-text-field>
                        </v-col>
                        <v-col cols="12" v-if="previewVideoUrl">
                            <v-row>
                                <v-col cols="12" class="d-flex justify-center">
                                    <div :style="'width: '+windowSize.width+'px; height: auto'">
                                        <v-img :src="previewVideoUrl" :height="windowSize.height + 'px'"
                                               class="grabbing" contain></v-img>
                                        <img ref="sourceVideoImage" :src="previewVideoUrl" style="display: none;">
                                    </div>
                                </v-col>
                            </v-row>
                        </v-col>
        ...
        <draggable v-model="backgrounds" :forceFallback="true" draggable=".item"
            ...
            <v-card v-for="(background, i) in backgrounds" :key="i" class="d-inline-block ma-1 item"
                ...
                <v-container v-if="background.type === 'image'" class="flex-column pa-0">
                    <v-container class="text-center pa-2 pb-0" style="width: 140px; height: 132px">
                        <v-img :src="background.url" class="handle align-center" width="140" height="132" contain></v-img>
                        <img ref="image" :src="background.url" style="display: none;">
                    </v-container>
                ...
                <v-container v-if="background.type === 'video'" class="flex-column pa-0">
                    <v-container class="text-center pa-2 pb-0 align-center" style="width: 140px; height: 132px">
                        <LazyYoutubeVideo ref="video" :videoId="background.url" contain
                                          :previewImageSize="previewImageSize"
                                          data-object-fit='cover' class="handle"/>
                    </v-container>
                    <v-container class="text-center px-0 py-0">
                        <v-container class="d-flex flex-row px-0 py-0" justify="center">
                            <v-container class="px-0 py-0">
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <v-btn v-on="on" icon @click.stop="editVideo(i)">
                                            <v-icon>mdi-image-edit</v-icon>
                                        </v-btn>
                                    </template>
                                    <span>{{ $t('edit') }}</span>
                            ...
                            <v-container class=" px-0 py-0">
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <v-btn v-on="on" icon @click.stop="removeVideo(i)">
                                            <v-icon>mdi-delete</v-icon>
                                        </v-btn>
                                    </template>
                                    <span>{{ $t('delete') }}</span>
            ...
            <v-card slot="footer" key="footer" class="d-inline-block flex-column mx-1 my-1"
                    style="vertical-align: top;" outlined elevation="3" width="140" height="170">
                <v-container class="fill-height py-0 justify-center">
                    <v-btn @click="addImage" class="ma-2" outlined block large>
                        <v-icon left>mdi-camera-plus-outline</v-icon>
                        {{ $t('photo') }}
                    </v-btn>
                    <v-btn @click="addVideo" class="ma-2" outlined block large>
                        <v-icon left>mdi-video-plus-outline</v-icon>
                        {{ $t('video') }}
                    </v-btn>
                </v-container>
            </v-card>
...
export default {
    ...
    methods: {
        ...
        editImage(i) {
            ...
            this.selectId = i
            let imageUrl = this.backgrounds[i].url
            if (this.cropper) {
                this.cropper.destroy()
            }
            if (this.objectUrl) {
        ...
        saveImage(i) {
            ...
            if (i === null && this.selectedImageFile.type) {
                ...
                if (mimeType) {
                    this.backgrounds[i].url = canvas.toDataURL(mimeType)
                }
            }
            this.cropper = null
            this.objectUrl = null
        ...
        setupVideoPreview(url) {
            let id = this.getIdVideo(url)
            if (id) {
                this.previewVideoUrl = `https://i.ytimg.com/vi/${id}/${this.previewImageSize}.jpg`
            } else {
        ...
        editVideo(i) {
            this.selectId = i
            this.selectIdVideo = this.backgrounds[i].url
            this.videoUrl = this.getUrlVideo(this.selectIdVideo)
            this.setupVideoPreview(this.videoUrl)
            this.dialogVideo = true
        },
        saveVideo(i) {
            if (i === null && this.videoUrl && !this.selectIdVideo) {
                let url = this.getIdVideo(this.videoUrl)
                this.backgrounds.push({
                    'type': 'video',
                    'url': url
                })
                ...
                this.backgrounds[i].url = this.getIdVideo(this.videoUrl)
            }
            this.selectId = null
            this.selectIdVideo = null
            this.videoUrl = null
